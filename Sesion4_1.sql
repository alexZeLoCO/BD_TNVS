...
FROM clientes AS A, empleados AS B
...
WHERE UPPER(empleados.ciudad) = 'OVIEDO' 
AND pedidos.fecha_pedido > '2008-09-01';

-- Consulta 14: Obtener todos los clientes (id_cliente y nombre) y la cantidad de pedidos que han solicitado.
SELECT clientes.id_cliente, clientes.nombre, COUNT(pedidos.id_pedido)
FROM clientes
LEFT JOIN pedidos USING (id_cliente)
GROUP BY id_cliente;

/* 
 * Consulta 15: Obtener todos los clientes y todos los empleados aunque no hubieran solicitado/servido ningún pedido. 
 * Mostrar nombre del cliente solicitante del pedido, nombre del empleado que sirvió el pedido y el identificador del pedido. 
 */
SELECT clientes.nombre, pedidos.id_pedido, empleados.nombre
FROM pedidos
FULL JOIN clientes USING (id_cliente)
FULL JOIN empleados USING (id_empleado);

-- Consulta 16: Mostrar los productos (identificador y nombre) que fueron servidos por el empleado ‘Juan’.
SELECT productos.id_producto, productos.nombre
FROM pedidos 
...
WHERE UPPER(empleados.nombre) LIKE 'JUAN';
...
INNER JOIN empleados ON pedidos.id_empleado = empleados.id_empleado
INNER JOIN productos ON detalles_pedido.id_producto = productos.id_producto
WHERE UPPER(empleados.nombre) NOT LIKE 'JUAN';


/* 
 * Consulta 18: Seleccionar los clientes cuyo nombre empiece por la letra ‘J’ o ‘j’. 
 * Investiga el uso de la orden LIKE
 */
SELECT *
FROM clientes
...
-- Consulta 21
-- TODO: Obtener clientes (id_cliente y nombre) que no han realizado ningún pedido. Usando LEFT JOIN, sin EXCEPT. 
SELECT clientes.id_cliente, clientes.nombre
FROM clientes
LEFT JOIN pedidos USING ()
